...
```bash
cloud_service$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
cloud_service$ sam local start-api
cloud_service$ curl http://localhost:3000/classify_digit
```
...
```bash
cloud_service$ sam logs -n InferenceFunction --stack-name cloud_service --tail
```
...
```bash
aws cloudformation delete-stack --stack-name cloud_service
```
